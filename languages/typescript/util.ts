...
function truthy(value) {
  if (typeof value === 'string') {
    return !['f', 'false', '0'].includes(value.toLowerCase())
  } else {
    return value
  }
}
...
function compact(obj: [any] | Object) {
...
function daysFrom(date, days) {
  return secondsFrom(date, days * SECONDS_PER_DAY)
}
...
function dateFormat(date) {
  const zeroPad = (digits) => ('0' + digits).slice(-2)
...
// See: http://ramdajs.com/docs/#invertObj
function invert(obj) {
  return Object.keys(obj).reduce((result, key) => {
    const value = obj[key]
    result[value] = key
