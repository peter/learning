# https://github.com/ericclack/pygamezero_pacman
import pgzrun
import random
...
def load_level(level):
  ...
  with open(filename) as f:
    for line in f:
      ...
      pacman.food_left += len([block for block in row if block == '.'])
      print(f'row: {row}')
      world.append(row)
...
def assert_valid_world():
  ...
  for i, row in enumerate(world):
    ...
    for j, block in enumerate(row):
      assert block in char_to_image or block == ' ', f'block at row {i} index {j} has invalid value "{block}"'
